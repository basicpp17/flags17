...
#define ENABLE_CLASSIC_FLAGS_OP(T)                                                                                     \
...
struct Flags {
    ...
    constexpr auto operator=(T e) noexcept -> This & {
        v = static_cast<ValueType>(e);
        return *this;
    }
    ...
    constexpr bool operator[](T t) const noexcept { return (v & t) != 0; }
    ...
    constexpr void each_set(F &&f) const noexcept {
        auto vt = ValueType{};
        while (true) {
            const auto t = static_cast<T>(1 << vt);
...
private:
    constexpr Flags(ValueType v) noexcept
        : v(v) {}

    template<class... Args>
    static constexpr auto build(Args... args) noexcept -> Flags {
        return Flags{(... | static_cast<ValueType>(args))};
    }
